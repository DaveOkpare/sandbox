...
describe('converter.ts', () => {
  ...
  describe('writeToolFiles', () => {
    ...
    it('should handle tools without output schemas', async () => {
      ...
      const writeFileCalls = mockWriteFile.mock.calls;
      const toolFileContent = writeFileCalls.find((call) => call[0].includes('sendNotification.ts'))?.[1];
    ...
    it('should include output interfaces when outputSchema is provided', async () => {
      ...
      const writeFileCalls = mockWriteFile.mock.calls;
      const toolFileContent = writeFileCalls.find((call) => call[0].includes('calculateStats.ts'))?.[1];
    ...
    it('should generate barrel export index.ts file', async () => {
      ...
      expect(indexFileContent).toContain('export { createProject, CreateProjectInput } from "./createProject.js"');
    });
  ...
  describe('Edge cases and integration', () => {
    it('should handle complex nested schema conversion and file writing', async () => {
      ...
      const writeFileCalls = mockWriteFile.mock.calls;
      const toolFileContent = writeFileCalls.find((call) => call[0].includes('createProject.ts'))?.[1];
